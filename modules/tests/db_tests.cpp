...
TEST_F(DBMS_Test, ConstraintUniqueIndexViolation) {
  ...
  ASSERT_TRUE(TableManager::createTable(path, TableSchema("unique_table", {c1}))
                  .isOk());
...
TEST_F(DBMS_Test, TypeMismatchProtection) {
  ...
  ASSERT_TRUE(
      TableManager::createTable(path, TableSchema("types_table", {c1})).isOk());
...
TEST_F(DBMS_Test, MultiIndexCreationAndInsertion) {
  ...
  ASSERT_TRUE(
      TableManager::createTable(path, TableSchema("multi_index", {c1, c2, c3}))
          .isOk());
...
TEST_F(DBMS_Test, AggregatesFull) {
  ...
  std::vector<AggregateRequest> reqs = {{AggregateType::COUNT, "*", "cnt"},
...
TEST_F(DBMS_Test, SlottedPagesCompaction) {
  ...
  ASSERT_TRUE(
      TableManager::createTable(path, TableSchema("compaction_test", {c1, c2}))
          .isOk());
...
TEST_F(DBMS_Test, StringIndexOverflowProtection) {
  ...
  ASSERT_TRUE(TableManager::createTable(path, TableSchema("str_overflow", {c1}))
                  .isOk());
...
TEST_F(DBMS_Test, MassUpdateRecords) {
  ...
  ASSERT_TRUE(
      TableManager::createTable(path, TableSchema("mass_update", {c1, c2, c3}))
          .isOk());

  for (int i = 1; i <= 5; ++i) {
    ASSERT_TRUE(
        TableManager::insertRow(path, {Value(i), Value("fruit"), Value(100)})
            .isOk());
  }
