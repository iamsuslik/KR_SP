...
protected:
    ...
    void TearDown() override {
        // Мы не удаляем данные здесь, чтобы можно было проверить файлы после падения
    }
...
TEST_F(DBMS_Test, ConstraintNotNullViolation) {
    ...
    ColumnDef c2("name", DataType::STR); c2.is_not_null = true;
    std::string path = getPath("strict_table");
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("strict_table", {c1, c2})).isOk());
...
TEST_F(DBMS_Test, ConstraintUniqueIndexViolation) {
    ColumnDef c1("uid", DataType::INT); c1.is_indexed = true;
    std::string path = getPath("unique_table");
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("unique_table", {c1})).isOk());
...
TEST_F(DBMS_Test, TypeMismatchProtection) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("types_table", {c1})).isOk());
...
TEST_F(DBMS_Test, MaxColumnsStability) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("fat_table", cols)).isOk());
}
...
TEST_F(DBMS_Test, MultiIndexCreationAndInsertion) {
    ...
    IndexKeyStr key{}; std::strncpy(key.data, "SECRET_CODE", TYPE_STR_SIZE-1);
...
TEST_F(DBMS_Test, MultiIndexDeleteSynchronization) {
    ColumnDef c1("id", DataType::INT); c1.is_indexed = true;
    ColumnDef c2("name", DataType::STR); c2.is_indexed = true;
    std::string path = getPath("sync_delete");
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("sync_delete", {c1, c2})).isOk());

    ASSERT_TRUE(TableManager::insertRow(path, {Value(1), Value("Alice")}).isOk());
    ...
    int count = 0;
    auto cb = [&](const std::string& s) { if(s.find('{') != std::string::npos) count++; };
    
    ASSERT_TRUE(TableManager::executeSelect(path, cond2.get(), {}, {}, {}, cb).isOk());
    EXPECT_EQ(count, 0); 
}
...
TEST_F(DBMS_Test, RecordRelocationSafety) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("reloc_test", {c1, c2})).isOk());

    ASSERT_TRUE(TableManager::insertRow(path, {Value(1), Value("small")}).isOk());

    std::string big_text(500, 'A'); 
    auto cond = std::make_unique<ComparisonNode>("id", "==", Value(1));
    std::vector<std::pair<std::string, Value>> upd = { {"text", Value(big_text)} };
    
    ASSERT_TRUE(TableManager::executeUpdate(path, cond.get(), upd).isOk());

    int count = 0;
    auto cb = [&](const std::string& s) { if(s.find('{') != std::string::npos) count++; };
    ASSERT_TRUE(TableManager::executeSelect(path, cond.get(), {}, {}, {}, cb).isOk());
    EXPECT_EQ(count, 1);
}

TEST_F(DBMS_Test, StringIndexRangeScan) {
    ColumnDef c1("tag", DataType::STR); c1.is_indexed = true;
    std::string path = getPath("str_range");
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("str_range", {c1})).isOk());
    ...
    std::string out;
    ASSERT_TRUE(TableManager::executeSelect(path, cond.get(), {}, {}, {}, [&](const std::string& s){ out += s; }).isOk());
...
TEST_F(DBMS_Test, ComplexLogicalExpressions) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("logic_test", {c1, c2})).isOk());
    ...
    auto term1 = std::make_unique<LogicalNode>("AND", 
        std::make_unique<ComparisonNode>("a", "==", Value(1)),
        std::make_unique<ComparisonNode>("b", "==", Value(2)));
    
    auto term2 = std::make_unique<LogicalNode>("AND", 
        std::make_unique<ComparisonNode>("a", "==", Value(2)),
        std::make_unique<ComparisonNode>("b", "==", Value(1)));

    auto final_cond = std::make_unique<LogicalNode>("OR", std::move(term1), std::move(term2));

    int count = 0;
    ASSERT_TRUE(TableManager::executeSelect(path, final_cond.get(), {}, {}, {}, [&](const std::string& s){
...
TEST_F(DBMS_Test, LikeOperatorRegex) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("like_test", {c1})).isOk());

    ASSERT_TRUE(TableManager::insertRow(path, {Value("Apple")}).isOk());
    ASSERT_TRUE(TableManager::insertRow(path, {Value("Application")}).isOk());
    ASSERT_TRUE(TableManager::insertRow(path, {Value("Banana")}).isOk());

    auto cond = std::make_unique<ComparisonNode>("name", "LIKE", Value("^App.*"));
    
    int count = 0;
    ASSERT_TRUE(TableManager::executeSelect(path, cond.get(), {}, {}, {}, [&](const std::string& s){
        if(s.find('{') != std::string::npos) count++;
    }).isOk());
...
TEST_F(DBMS_Test, AggregatesFull) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("agg_full", {c1})).isOk());

    for(int i = 1; i <= 10; ++i) ASSERT_TRUE(TableManager::insertRow(path, {Value(i)}).isOk());
    ...
    std::string output;
    ASSERT_TRUE(TableManager::executeSelect(path, nullptr, {}, {}, reqs, [&](const std::string& s){ output += s; }).isOk());
...
TEST_F(DBMS_Test, AggregatesOnEmptyTable) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("empty_agg", {c1})).isOk());
...
TEST_F(DBMS_Test, AccessLoggerWritesToFile) {
    ...
    std::ifstream file(log_file); // Теперь работает, т.к. добавлен инклуд <fstream>
...
TEST_F(DBMS_Test, AsyncManagerSlotAllocation) {
    ...
    async.update_task(guid1, AsyncStatus::COMPLETED, StatusCode::OK, test_result);
...
TEST_F(DBMS_Test, BPlusTreeDeepStressInsert) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("stress_insert", {c1})).isOk());
    ...
    Pager p(path); TableHeader h; ASSERT_TRUE(p.read_page(0, &h).isOk());
    TablePageManager pm(p, h);
...
TEST_F(DBMS_Test, BPlusTreeStressDelete) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("stress_delete", {c1})).isOk());
...
TEST_F(DBMS_Test, SlottedPagesCompaction) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("compaction_test", {c1, c2})).isOk());

    std::string big_string(1000, 'X'); 
    ASSERT_TRUE(TableManager::insertRow(path, {Value(1), Value(big_string)}).isOk());
    ASSERT_TRUE(TableManager::insertRow(path, {Value(2), Value(big_string)}).isOk());
    ASSERT_TRUE(TableManager::insertRow(path, {Value(3), Value(big_string)}).isOk());

    auto cond_del = std::make_unique<ComparisonNode>("id", "==", Value(2));
    ASSERT_TRUE(TableManager::executeDelete(path, cond_del.get()).isOk());

    Result res = TableManager::insertRow(path, {Value(4), Value(std::string(800, 'Y'))});
    ASSERT_TRUE(res.isOk());
    EXPECT_EQ(res.rid.page_id, 1);
}

TEST_F(DBMS_Test, StringIndexOverflowProtection) {
    ColumnDef c1("code", DataType::STR); c1.is_indexed = true;
    std::string path = getPath("str_overflow");
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("str_overflow", {c1})).isOk());
...
TEST_F(DBMS_Test, SelectWithAliases) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("alias_test", {c1, c2})).isOk());

    ASSERT_TRUE(TableManager::insertRow(path, {Value(1), Value("John")}).isOk());

    std::map<std::string, std::string> aliases = { {"id", "user_id"}, {"first_name", "name"} };
    std::string output;
    ASSERT_TRUE(TableManager::executeSelect(path, nullptr, {"id", "first_name"}, aliases, {}, [&](const std::string& s) {
        output += s;
    }).isOk());
...
TEST_F(DBMS_Test, MassUpdateRecords) {
    ...
    ASSERT_TRUE(TableManager::createTable(path, TableSchema("mass_update", {c1, c2, c3})).isOk());

    for (int i = 1; i <= 5; ++i) {
        ASSERT_TRUE(TableManager::insertRow(path, {Value(i), Value("fruit"), Value(100)}).isOk());
    }

    auto cond = std::make_unique<ComparisonNode>("category", "==", Value("fruit"));
    std::vector<std::pair<std::string, Value>> assignments = { {"price", Value(150)} };
    
    ASSERT_TRUE(TableManager::executeUpdate(path, cond.get(), assignments).isOk());

    int count = 0;
    auto cond_check = std::make_unique<ComparisonNode>("price", "==", Value(150));
    ASSERT_TRUE(TableManager::executeSelect(path, cond_check.get(), {}, {}, {}, [&](const std::string& s){
        if (s.find('{') != std::string::npos) count++;
    }).isOk());
...
TEST_F(DBMS_Test, RBACGroupPermissions) {
    ASSERT_TRUE(TableManager::insertRow(getPath("_system.users"), {Value("worker"), Value("hash"), Value("salt")}).isOk());
    ASSERT_TRUE(TableManager::insertRow(getPath("_system.groups"), {Value("editors"), Value("test_db")}).isOk());
    ASSERT_TRUE(TableManager::insertRow(getPath("_system.user_groups"), {Value("worker"), Value("editors"), Value("test_db")}).isOk());
    ASSERT_TRUE(TableManager::insertRow(getPath("_system.permissions"), {Value("editors"), Value(1), Value("test_db"), Value("WRITE")}).isOk());
