...
uint16_t RecordManager::calculateSize(const Row& row,
...
std::vector<char> RecordManager::serializeRowDynamic(
...
Row RecordManager::extractRowDynamic(const char* record_ptr,
                                     const TableHeader& header) {
    Row row;
    uint16_t offset = 0;
    ...
    for (uint32_t i = 0; i < header.column_count; ++i) {
        if (!(bitmap & (1 << i))) {
            ...
            if (header.columns[i].type == static_cast<uint8_t>(DataType::INT)) {
                ...
                offset += sizeof(uint32_t);
                if (offset + str_len > PAGE_SIZE)
                    throw std::runtime_error(
                        "Коррупция страницы: длина строки выходит за пределы "
                        "PAGE_SIZE");
...
bool RecordManager::isPageEmpty(const char* page_buffer) {
    ...
    const auto* slots =
        reinterpret_cast<const Slot*>(page_buffer + sizeof(PageHeader));
    ...
    *new_hdr = *old_hdr;
    ...
    for (uint16_t i = 0; i < old_hdr->slot_count; ++i) {
        if (old_slots[i].length > 0 && old_slots[i].offset > 0) {
            ...
            std::memcpy(temp + new_hdr->free_ptr,
                        page_buffer + old_slots[i].offset, old_slots[i].length);
    ...
    uint16_t slots_area_end =
        sizeof(PageHeader) + (new_hdr->slot_count * sizeof(Slot));
...
void RecordManager::initColumnSchema(ColumnSchema& dest, const ColumnDef& src) {
    ...
    dest.type =
        (src.type == DataType::INT) ? static_cast<uint8_t>(DataType::INT) : 1;
