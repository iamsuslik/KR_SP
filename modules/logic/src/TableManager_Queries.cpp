...
bool TableManager::executePointQuery(
  ...
  std::cerr << "[Node " << g_current_node_id << "] Optimizer: Point Query on '"
            << comp_node->column_name << "'\n";
  ...
  if (res.isOk()) {
    ...
    if (!row.empty()) {
      if (!isAgg) callback("[\n");
      processRow(row, header, cond, aggs, colsToPrint, aliases, t_sum, t_count,
                 first, isAgg, callback);
      if (!isAgg) callback("\n]\n");
    } else {
      if (!isAgg)
        callback("[]\n");
...
bool TableManager::executeRangeScan(
  ...
  if (auto* between = dynamic_cast<const BetweenNode*>(cond)) {
    ...
    if (header.columns[colIdx].type == static_cast<uint8_t>(DataType::INT)) {
      ...
      tree.range_scan(between->low.int_val, between->high.int_val, processFunc);
    } else {
      ...
      std::strncpy(lo.data, between->low.str_val.c_str(), TYPE_STR_SIZE - 1);
      std::strncpy(hi.data, between->high.str_val.c_str(), TYPE_STR_SIZE - 1);
  ...
  if (auto* comp = dynamic_cast<const ComparisonNode*>(cond)) {
    ...
    std::cerr << "[Node " << g_current_node_id << "] Optimizer: Range Scan ("
              << comp->op << ") on '" << comp->column_name << "'\n";
...
void TableManager::executeTreeScan(
  ...
  if (colIdx != -1 && header.root_page_ids[colIdx] != 0) {
    std::cerr << "[Node " << g_current_node_id << "] Optimizer: Index Scan on '"
...
Result TableManager::executeSelect(
  ...
  try {
    ...
    if (!isAgg) callback("[\n");
    bool used_range =
        executeRangeScan(pager, header, cond, colsToPrint, aliases, aggs, t_sum,
                         t_count, first, isAgg, callback);

    if (!used_range) {
      executeTreeScan(pager, header, cond, colsToPrint, aliases, aggs, t_sum,
                      t_count, first, isAgg, callback);
    }
    if (!isAgg) callback("\n]\n");
