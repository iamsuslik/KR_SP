...
Result TableManager::createTable(const std::string& full_path,
    ...
    if (schema.columns.size() > MAX_COLUMNS) {
        return Result::Error(StatusCode::INVALID_VALUE,
                             "Ошибка: Таблица не может иметь более " +
                                 std::to_string(MAX_COLUMNS) + " колонок");
    }
    try {
        ...
        for (size_t i = 0; i < schema.columns.size() && i < MAX_COLUMNS; ++i) {
            RecordManager::initColumnSchema(header.columns[i],
                                            schema.columns[i]);
        }
...
Result TableManager::getRIDFromIndex(Pager& pager, TableHeader& header,
    ...
    if (!comp_node) {
        return Result::Error(StatusCode::NOT_FOUND,
                             "Condition is not a simple comparison");
    }

    if (comp_node->op != "==" && comp_node->op != "=") {
        return Result::Error(StatusCode::NOT_FOUND,
                             "Operator not supported by index");
    }
    ...
    if (colIdx == -1) {
        return Result::Error(
            ...
            "Column '" + comp_node->column_name + "' not found");
    }

    if (!header.columns[colIdx].is_indexed ||
        header.root_page_ids[colIdx] == 0) {
        return Result::Error(
            StatusCode::NOT_FOUND,
            "No index on column '" + comp_node->column_name + "'");
    }
    ...
    try {
        if (header.columns[colIdx].type ==
            ...
            IndexKeyStr key{};
            std::memset(key.data, 0, TYPE_STR_SIZE);
            std::strncpy(key.data, comp_node->literal_value.str_val.c_str(),
                         TYPE_STR_SIZE - 1);
            return index.find(key, out_rid);
        }
    } catch (const DbException& e) {
        return Result::Error(e.code(),
                             std::string("Index internal error: ") + e.what());
    } catch (const std::exception& e) {
...
Result TableManager::insertRow(const std::string& full_path, const Row& row) {
    ...
    try {
        if (!TableLockManager::lock_table(fd_to_unlock, true)) {
            return Result::Error(StatusCode::IO_ERROR,
                                 "Не удалось заблокировать таблицу для записи");
        }
        ...
        for (uint32_t i = 0; i < header.column_count; ++i) {
            if (row[i].is_null && header.columns[i].is_not_null) {
                ...
                return Result::Error(StatusCode::NOT_NULL_VIOLATION,
                                     "Нарушение NOT NULL");
            }

            if (!row[i].is_null) {
                bool is_col_int = (header.columns[i].type ==
                                   static_cast<uint8_t>(DataType::INT));
                ...
                if (is_col_int != is_val_int) {
                    ...
                    throw DbException(StatusCode::TYPE_MISMATCH,
            ...
            if (header.columns[i].is_indexed && row[i].type == DataType::STR) {
                if (row[i].str_val.size() >= TYPE_STR_SIZE) {
                    TableLockManager::unlock_table(fd_to_unlock);
                    return Result::Error(StatusCode::INVALID_VALUE,
                                         "Строка слишком велика для индекса");
                }
            }
        }
        ...
        if (rid.page_id == 0) {
            throw DbException(StatusCode::OUT_OF_MEMORY,
        ...
        try {
            if (col.is_indexed) {
                if (col.type == static_cast<uint8_t>(DataType::INT)) {
                    ...
                    BP_tree<IndexKeyStr> index(
                        pager, header.root_page_ids[colIdx], pm);
                    IndexKeyStr oldK{}, newK{};
                    std::memset(oldK.data, 0, TYPE_STR_SIZE);
                    std::memset(newK.data, 0, TYPE_STR_SIZE);
                    std::strncpy(oldK.data, row[colIdx].str_val.c_str(),
                                 TYPE_STR_SIZE - 1);
                    std::strncpy(newK.data, newValue.str_val.c_str(),
                                 TYPE_STR_SIZE - 1);
...
Result TableManager::executeUpdate(
    ...
    try {
        ...
        if (idx_res.isOk()) {
            ...
            Row old_row =
                getRowFromSlot(buf, static_cast<uint16_t>(rid.slot_id), header);
            if (!old_row.empty()) {
                Row updated_row = old_row;
                bool h_changed = false;

                applyAssignments(updated_row, assignments, header, pager, rid,
                                 h_changed);
...
int TableManager::fullScanUpdate(
    ...
    for (uint32_t p = 1; p < pager.get_page_count(); ++p) {
        ...
        for (uint16_t i = 0; i < scount; ++i) {
            ...
            Row updated_row = old_row;
            bool h_changed = false;
            applyAssignments(updated_row, assignments, header, pager, {p, i},
                             h_changed);
            handleRecordUpdate(pager, header, page_buffer, p, i, updated_row,
                               old_row);
...
Result TableManager::executeDelete(const std::string& full_path,
    ...
    try {
        if (!TableLockManager::lock_table(fd_to_unlock, true)) {
            return Result::Error(
                StatusCode::IO_ERROR,
                "Не удалось заблокировать таблицу для удаления (файл занят)");
        }
        ...
        if (idx_res.isOk()) {
            ...
            if (row.empty()) {
                ...
                return Result::Success({0, 0},
                                       "Удалено 0 строк (Запись уже пуста)");
            }
            ...
            // Физически записываем изменения. Если диск «отвалится», мы узнаем
            // об этом здесь.
            pager.write_page(rid.page_id, buf).throw_if_error();
            pager.write_page(0, &header).throw_if_error();
            ...
            TableLockManager::unlock_table(fd_to_unlock);
            return Result::Success({0, 0},
                                   "Успешно удалена 1 строка (через индекс)");
        }

        // Если индекс вернул ошибку, которая НЕ является "NOT_FOUND" (например,
        // ошибка I/O в дереве)
        ...
        TableLockManager::unlock_table(fd_to_unlock);
        return Result::Success(
            {0, 0}, "Удалено " + std::to_string(count) + " строк (Full Scan)");
...
void TableManager::handleRecordUpdate(Pager& pager, TableHeader& header,
                                      char* page_buf, uint32_t p_id,
                                      uint16_t slot_id, const Row& new_row,
                                      const Row& old_row) {
    std::vector<char> new_rec =
        RecordManager::serializeRowDynamic(new_row, header);
    ...
    if (page_has_space(page_buf, new_size)) {
        ...
        relocate_record(pager, header, page_buf, slot_id, new_rec, new_row,
                        old_row);
    }
}

void TableManager::recalc_free_space(PageHeader* hdr) {
    uint16_t slots_end = static_cast<uint16_t>(sizeof(PageHeader) +
                                               hdr->slot_count * sizeof(Slot));
    hdr->free_space =
        (hdr->free_ptr > slots_end) ? (hdr->free_ptr - slots_end) : 0;
}

bool TableManager::page_has_space(const char* buf, uint16_t rec_size) {
    return get_hdr(buf)->free_space >=
           (rec_size + static_cast<uint16_t>(sizeof(Slot)));
}

uint16_t TableManager::write_record_to_page(char* page_buffer,
...
void TableManager::relocate_record(Pager& pager, TableHeader& header,
    ...
    RecordID new_rid =
        findAvailableSlot(pager, header, static_cast<uint16_t>(new_rec.size()));
