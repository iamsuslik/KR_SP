...
Result TableManager::getRIDFromIndex(Pager& pager, TableHeader& header,
  ...
  if (colIdx == -1) {
    return Result::Error(StatusCode::COLUMN_NOT_FOUND,
...
Result TableManager::insertRow(const std::string& full_path, const Row& row) {
  ...
  try {
    ...
    for (uint32_t i = 0; i < header.column_count; ++i) {
      ...
      if (!row[i].is_null) {
        bool is_col_int =
            (header.columns[i].type == static_cast<uint8_t>(DataType::INT));
    ...
    if (idx_res.isOk()) {
      ...
      if (!old_row.empty()) {
        ...
        handleRecordUpdate(pager, header, buf, rid.page_id,
                           static_cast<uint16_t>(rid.slot_id), updated_row,
                           old_row);
...
Result TableManager::executeDelete(const std::string& full_path,
  ...
  try {
    ...
    if (idx_res.isOk()) {
      ...
      // Физически записываем изменения. Если диск «отвалится», мы узнаем об
      // этом здесь.
      pager.write_page(rid.page_id, buf).throw_if_error();
      pager.write_page(0, &header).throw_if_error();
