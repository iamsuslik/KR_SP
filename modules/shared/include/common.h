...
struct Value {
  ...
  explicit Value(std::string v)
      : type(DataType::STR), str_val(std::move(v)), is_null(false) {}
  ...
  static bool compare(const Value& lhs, const Value& rhs,
    ...
    if (lhs.type == DataType::INT) {
      return Engine::RelationalOp<int, IntComparator>::eval(lhs.int_val,
                                                            rhs.int_val, op);
    } else {
      if (op == "LIKE")
        return Engine::StringUtils::matchLike(lhs.str_val, rhs.str_val);
      return Engine::RelationalOp<std::string, std::less<std::string>>::eval(
          lhs.str_val, rhs.str_val, op);
    }
  }
};
...
struct TableSchema {
  ...
  TableSchema(std::string n, std::vector<ColumnDef> c)
      : name(std::move(n)), columns(std::move(c)) {}
};
...
struct TableHeader {
  ...
  char padding[PAGE_SIZE -
               ((sizeof(uint32_t) * 3) + (sizeof(uint32_t) * MAX_FREE_PAGES) +
...
struct [[nodiscard]] Result {
  ...
  static Result Success(RecordID r = {0, 0}, std::string d = "") {
    return {StatusCode::OK, d, r};
  }
  static Result Error(StatusCode c, std::string d = "") {
    return {c, std::move(d)};
  }
};
