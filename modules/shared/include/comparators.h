...
struct StrComparator {
    bool operator()(const IndexKeyStr& lhs,
                    const IndexKeyStr& rhs) const noexcept {
        return lhs < rhs;
    }
};
...
struct RelationalOp {
    static bool eval(const T& lhs, const T& rhs,
...
struct RelationalOp<std::string, std::less<std::string>> {
    static bool eval(const std::string& lhs, const std::string& rhs,
                     const std::string& op) noexcept {
        if (op == "LIKE") {
    ...
    static bool StringLike(const std::string& text,
...
struct StringUtils {
    static bool matchLike(const std::string& text,
                          const std::string& pattern) noexcept {
        return RelationalOp<std::string, std::less<std::string>>::StringLike(
            text, pattern);
    }
};
