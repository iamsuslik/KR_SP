...
std::string AuthManager::hashPassword(const std::string& password,
    ...
    ss << std::hex << std::setw(16) << std::setfill('0') << h1 << std::setw(16)
       << std::setfill('0') << h2;
    return ss.str();
}
...
std::string AuthManager::createToken(const std::string& username) {
    ...
    json payload_obj = {{"user", username}, {"iat", static_cast<int64_t>(now)}};
...
std::string AuthManager::verifyToken(const std::string& token) {
    ...
    if (fd == std::string::npos || ld == std::string::npos || fd == ld) {
        throw DbException(StatusCode::AUTH_FAILED,
...
void AuthManager::initSystem(HierarchyManager& hm) {
    ...
    {
        std::vector<ColumnDef> c = {ColumnDef("login", DataType::STR),
    ...
    {
        std::vector<ColumnDef> c = {ColumnDef("group_name", DataType::STR),
    ...
    {
        std::vector<ColumnDef> c = {ColumnDef("username", DataType::STR),
    ...
    {
        std::vector<ColumnDef> c = {ColumnDef("grantee", DataType::STR),
...
bool AuthManager::authenticate(const std::string& username,
    ...
    auto cond =
        std::make_unique<ComparisonNode>("login", "==", Value(username));
...
bool AuthManager::checkAccess(const std::string& username,
    ...
    if (has_match(perm_path.path,
                  std::make_unique<LogicalNode>(
                      ...
                      std::make_unique<ComparisonNode>(
                          "grantee", "==", Value(std::string("PUBLIC"))),
                      std::make_unique<LogicalNode>(
                          "AND",
                          std::make_unique<ComparisonNode>(
                              "db_name", "==", Value(resource)),
                          std::make_unique<ComparisonNode>(
                              "action", "==", Value(action))))))
        return true;
    ...
    std::vector<std::string> groups;
    {
        ...
        auto cb = [&](const std::string& s) { raw += s; };
        auto gc =
            std::make_unique<ComparisonNode>("username", "==", Value(username));
