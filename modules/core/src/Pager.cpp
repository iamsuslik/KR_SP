...
Pager::Pager(const std::filesystem::path& db_path)
    : filename(db_path), fd(-1), page_count(0) {
    if (db_path.empty()) {
        throw std::invalid_argument(
            "Pager Error: Database path cannot be empty.");
    }
    ...
    if (fd == -1) {
        throw std::runtime_error(
            "Pager Fatal: Could not open or create file: " + filename.string());
    }

    struct stat st;
    if (fstat(fd, &st) == -1) {
        ::close(fd);
        throw std::runtime_error("Pager Fatal: Failed to stat file: " +
                                 filename.string());
    }
...
Result Pager::read_page(uint32_t page_id, void* buffer) {
    if (page_id >= page_count) {
        return Result::Error(
            StatusCode::IO_ERROR,
            "Pager Error: Out-of-bounds (ID: " + std::to_string(page_id) + ")");
    }

    ssize_t bytes_read =
        ::pread(fd, buffer, PAGE_SIZE, get_page_offset(page_id));

    if (bytes_read != PAGE_SIZE) {
        return Result::Error(StatusCode::IO_ERROR,
                             "Pager Error: Physical read failed (ID: " +
...
Result Pager::write_page(uint32_t page_id, const void* buffer) {
    if (page_id >= page_count) {
        return Result::Error(StatusCode::IO_ERROR,
                             "Pager Error: Cannot write to unallocated page " +
                                 std::to_string(page_id));
    }

    ssize_t bytes_written =
        ::pwrite(fd, buffer, PAGE_SIZE, get_page_offset(page_id));

    if (bytes_written != PAGE_SIZE) {
        return Result::Error(StatusCode::IO_ERROR,
                             "Pager Error: Physical write failed at page " +
                                 std::to_string(page_id));
    }
