...
void TelemetryManager::cleanup_unsafe() {
    ...
    while (!history.empty()) {
        auto age = std::chrono::duration_cast<std::chrono::seconds>(
            now - history.front().timestamp);
        if (age > limit) {
            history.pop_front();
        } else {
            break;  // Остальные данные еще актуальны (deque упорядочен по
                    // времени)
        }
    }
}
...
void TelemetryManager::recordQuery(long long duration_ms, bool is_error) {
    ...
    history.push_back(
        {std::chrono::steady_clock::now(), duration_ms, is_error});
    cleanup_unsafe();
}
...
double TelemetryManager::getCurrentRPS(int seconds) const {
    ...
    auto now = std::chrono::steady_clock::now();
    auto count =
        std::count_if(history.begin(), history.end(), [&](const auto& m) {
...
double TelemetryManager::getAvgRPS(int minutes) const {
    ...
    // Считаем фактический интервал времени в истории (но не более заданных
    // минут)
    auto now = std::chrono::steady_clock::now();
    ...
    double target_period = std::min(static_cast<double>(duration_sec),
...
double TelemetryManager::getMaxRPS(int minutes) const {
    ...
    for (const auto& m : history) {
        auto sec_ago =
            std::chrono::duration_cast<std::chrono::seconds>(now - m.timestamp)
                .count();
...
double TelemetryManager::getErrorRate(int minutes) const {
    ...
    for (const auto& m : history) {
        if (std::chrono::duration_cast<std::chrono::minutes>(now - m.timestamp)
                .count() < minutes) {
            total++;
...
double TelemetryManager::getAvgDuration(int seconds) const {
    ...
    for (const auto& m : history) {
        if (std::chrono::duration_cast<std::chrono::seconds>(now - m.timestamp)
                .count() < seconds) {
...
void TelemetryManager::printMetrics() const {
    ...
    std::cout << "\n"
              ...
              << std::left << std::setw(LABEL_COLUMN_WIDTH) << "Average RPS ("
              << METRIC_RPS_WINDOW_MIN << "m):" << std::fixed
              << std::setprecision(FLOAT_PRECISION_RPS)
              << getAvgRPS(METRIC_RPS_WINDOW_MIN) << "\n"

              << std::left << std::setw(LABEL_COLUMN_WIDTH) << "Peak RPS ("
              << METRIC_RPS_WINDOW_MIN
              << "m):" << getMaxRPS(METRIC_RPS_WINDOW_MIN) << "\n"

              << std::left << std::setw(LABEL_COLUMN_WIDTH) << "Avg Latency ("
              << METRIC_LATENCY_WINDOW_SEC
              << "s):" << getAvgDuration(METRIC_LATENCY_WINDOW_SEC) << " ms\n"

              << std::left << std::setw(LABEL_COLUMN_WIDTH) << "Error Rate ("
              << METRIC_ERROR_WINDOW_MIN << "m):" << std::fixed
              << std::setprecision(FLOAT_PRECISION_ERROR)
