...
template <typename tkey, std::size_t t = get_optimal_t<tkey>(),
...
class BP_tree final : private compare {
   ...
   private:
    ...
    inline bool equal(const tkey& lhs, const tkey& rhs) const {
...
#pragma pack(push, 1)
    ...
    struct bptree_node_term : public bptree_node_base {
        ...
        char _padding[PAGE_SIZE - sizeof(bptree_node_base) - sizeof(uint32_t) -
                      (sizeof(tree_data_type) * (maximum_keys_in_node + 1))];
    ...
    struct bptree_node_middle : public bptree_node_base {
        ...
        char _padding[PAGE_SIZE - sizeof(bptree_node_base) -
                      (sizeof(tkey) * (maximum_keys_in_node + 1)) -
                      (sizeof(uint32_t) * (maximum_keys_in_node + 2))];
...
#pragma pack(pop)
   ...
   public:
    explicit BP_tree(Pager& pager, uint32_t& root_ref, IPageManager& pm,
    ...
    void range_scan(const tkey& lo, const tkey& hi,
                    std::function<void(const RecordID&)> callback) const;
    void lower_bound_scan(const tkey& start,
                          std::function<void(const RecordID&)> callback) const;
   ...
   private:
    ...
    size_t binary_search_in_node(const bptree_node_base* node,
    ...
    void write_node(uint32_t page_id, const void* buffer) {
        _pager.write_page(page_id, buffer).throw_if_error();
    }
};

template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
uint32_t BP_tree<tkey, t, compare>::find_path(const tkey& key,
...
template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
size_t BP_tree<tkey, t, compare>::binary_search_in_node(
    const bptree_node_base* node, const tkey& key) const {
    size_t l = 0;
...
template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
Result BP_tree<tkey, t, compare>::find(const tkey& key,
                                       RecordID& out_rid) const {
    if (_root_id == 0) {
        return Result::Error(StatusCode::NOT_FOUND,
                             "search error: b+-tree is empty");
    }
...
template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
Result BP_tree<tkey, t, compare>::lower_bound(const tkey& key,
                                              RecordID& out_rid) const {
    if (_root_id == 0) {
        return Result::Error(StatusCode::NOT_FOUND,
                             "lower_bound error: b+-tree is empty");
    }
    ...
    return Result::Error(StatusCode::NOT_FOUND,
                         "lower_bound: no elements >= key found");
}

template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
void BP_tree<tkey, t, compare>::range_scan(
    const tkey& lo, const tkey& hi,
    std::function<void(const RecordID&)> callback) const {
    if (_root_id == 0) return;
...
void BP_tree<tkey, t, compare>::lower_bound_scan(
    const tkey& start, std::function<void(const RecordID&)> callback) const {
    if (_root_id == 0) return;
...
template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
Result BP_tree<tkey, t, compare>::upper_bound(const tkey& key,
                                              RecordID& out_rid) const {
    if (_root_id == 0) {
        return Result::Error(StatusCode::NOT_FOUND,
                             "upper_bound error: b+-tree is empty");
    }
    ...
    return Result::Error(StatusCode::NOT_FOUND,
                         "upper_bound: no element strictly greater than key");
}

template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
void BP_tree<tkey, t, compare>::split_node(uint32_t node_id,
                                           uint32_t parent_id) {
    alignas(PAGE_SIZE) char buffer[PAGE_SIZE];
...
template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
void BP_tree<tkey, t, compare>::split_leaf(uint32_t leaf_id,
                                           uint32_t parent_id) {
    alignas(PAGE_SIZE) char l_buf[PAGE_SIZE], r_buf[PAGE_SIZE],
        p_buf[PAGE_SIZE];
...
template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
void BP_tree<tkey, t, compare>::split_middle(uint32_t mid_id,
                                             uint32_t parent_id) {
    alignas(PAGE_SIZE) char m_buf[PAGE_SIZE], n_buf[PAGE_SIZE],
        p_buf[PAGE_SIZE];
...
template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
bool BP_tree<tkey, t, compare>::is_node_full(
    const bptree_node_base* node) const {
    return node->_count > maximum_keys_in_node;
}

template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
void BP_tree<tkey, t, compare>::balance_insert(uint32_t curr_id,
                                               std::vector<uint32_t>& path) {
    alignas(PAGE_SIZE) char buffer[PAGE_SIZE];
    while (true) {
        _pager.read_page(curr_id, buffer).throw_if_error();
        if (reinterpret_cast<bptree_node_base*>(buffer)->_count <=
    ...
    if (_root_id == 0) {
        ...
        if (new_root_id == 0) {
            return Result::Error(
                ...
                "Не удалось выделить страницу для корня индекса");
        }
...
Result BP_tree<tkey, t, compare>::erase(const tkey& key_del) {
    if (_root_id == 0)
        return Result::Error(StatusCode::NOT_FOUND, "Tree empty");
...
template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
bool BP_tree<tkey, t, compare>::is_node_underfull(
    const bptree_node_base* node) const {
...
void BP_tree<tkey, t, compare>::shrink_root() {
    ...
    // Если в корневом узле не осталось ключей (все данные ушли в потомков после
    // удаления)
...
template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
void BP_tree<tkey, t, compare>::balance_delete(uint32_t curr_id,
                                               std::vector<uint32_t>& path) {
    alignas(PAGE_SIZE) char buffer[PAGE_SIZE];
...
template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
bool BP_tree<tkey, t, compare>::borrow_sibling(uint32_t curr_id,
                                               uint32_t parent_id) {
    alignas(PAGE_SIZE) char c_buf[PAGE_SIZE], p_buf[PAGE_SIZE],
        s_buf[PAGE_SIZE];
    ...
    if (idx > 0) {
        ...
        if (sib->_count > minimum_keys_in_node) {
            if (curr->_is_terminate) {
                ...
                parent->_keys[idx - 1] =
                    std::move(s_mid->_keys[sib->_count - 1]);
            }
            curr->_count++;
            sib->_count--;
            _pager.write_page(curr_id, c_buf).throw_if_error();
...
template <typename tkey, std::size_t t, typename compare>
    requires comparator<compare, tkey>
void BP_tree<tkey, t, compare>::merge_sibling(uint32_t curr_id,
                                              uint32_t parent_id) {
    alignas(PAGE_SIZE) char l_buf[PAGE_SIZE], r_buf[PAGE_SIZE],
        p_buf[PAGE_SIZE];
