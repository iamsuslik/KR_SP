...
class Statement {
    ...
    Result requireAccess(const std::string& token, const std::string& resource,
        ...
        if (!AuthManager::checkAccess(user, resource, action, hm)) {
            return Result::Error(StatusCode::PERMISSION_DENIED,
                                 "Отказ в доступе: требуется право '" + action +
                                     "' для '" + resource + "'");
        }
        return Result::Success();
    }

    static std::string db_of(const std::string& table,
   ...
   public:
    ...
    virtual Result execute(HierarchyManager& hm, const std::string& token,
                           OutputCallback cb) = 0;
};
...
class CreateTableStatement : public Statement {
   ...
   public:
    ...
    Result execute(HierarchyManager& hm, const std::string& token,
        ...
        Result r =
            TableManager::createTable(res.path, TableSchema(table_, cols_));
        if (r.isOk()) {
...
class DropTableStatement : public Statement {
   ...
   public:
    ...
    Result execute(HierarchyManager& hm, const std::string& token,
        ...
        if (r.isOk())
            cb("[Success] Таблица '" + table_ + "' удалена.\n");
        else
            cb("[Error] " + r.details + "\n");
        return r;
    }
...
class InsertStatement : public Statement {
   ...
   public:
    InsertStatement(std::string t, std::vector<std::string> tc,
    ...
    Result execute(HierarchyManager& hm, const std::string& token,
        ...
        if (!target_cols_.empty()) {
            for (size_t i = 0; i < target_cols_.size() && i < rawValues_.size();
                 ++i) {
                int ci = find_col(header, target_cols_[i]);
                if (ci == -1) {
                    cb("[Error] Колонка '" + target_cols_[i] +
                       "' не найдена.\n");
   ...
   private:
    static int find_col(const TableHeader& h, const std::string& name) {
        for (uint32_t i = 0; i < h.column_count; ++i)
            if (std::string(h.columns[i].name) == name)
                return static_cast<int>(i);
        return -1;
    }

    static bool apply_constraints(Row& row, const TableHeader& h,
                                  OutputCallback cb) {
        for (uint32_t i = 0; i < h.column_count; ++i) {
            ...
            if (h.columns[i].has_default) {
                ...
                cb("[Error] Колонка '" + std::string(h.columns[i].name) +
                   "' NOT_NULL.\n");
                return false;
            }
        }
...
class InsertSelectStatement : public Statement {
   ...
   public:
    InsertSelectStatement(std::string t, std::vector<std::string> c,
                          std::unique_ptr<Statement> s)
        : table_(std::move(t)),
          target_cols_(std::move(c)),
    ...
    Result execute(HierarchyManager& hm, const std::string& token,
        ...
        if (!dst_res.isOk()) {
            cb("[Error] " + dst_res.details + "\n");
            return dst_res;
        }

        TableHeader dst_header;
        try {
            Pager(dst_res.path).read_page(0, &dst_header).throw_if_error();
        } catch (const DbException& e) {
            cb("[Error] " + std::string(e.what()) + "\n");
            return Result::Error(e.code(), e.what());
        }
        ...
        auto intercept_cb = [&](const std::string& chunk) {
            select_buf += chunk;
        };

        Result sel_res = select_query_->execute(hm, token, intercept_cb);
        if (!sel_res.isOk()) {
            cb("[Error] " + sel_res.details + "\n");
            return sel_res;
        }
        ...
        for (const auto& src_row : src_rows) {
            ...
            if (!ok) continue;
            if (TableManager::insertRow(dst_res.path, new_row).isOk())
                ++inserted_count;
        }

        cb("[Success] Скопировано " + std::to_string(inserted_count) +
           " строк.\n");
        return Result::Success();
    }
...
class SelectStatement : public Statement {
   ...
   public:
    SelectStatement(std::string t, std::vector<std::string> c,
    ...
    Result execute(HierarchyManager& hm, const std::string& token,
        ...
        return TableManager::executeSelect(res.path, where_.get(), cols_,
                                           aliases_, aggs_, cb);
    }
};
...
class UpdateStatement : public Statement {
   ...
   public:
    UpdateStatement(std::string t, std::vector<std::pair<std::string, Value>> a,
                    std::unique_ptr<ASTNode> w)
...
class AuthStatement : public Statement {
   ...
   public:
    ...
    Result execute(HierarchyManager& hm, const std::string&,
                   OutputCallback cb) override {
...
class AuthJwtStatement : public Statement {
   ...
   public:
    ...
    Result execute(HierarchyManager&, const std::string&,
                   OutputCallback cb) override {
        try {
            std::string user = AuthManager::verifyToken(token_);
            json resp = {
                {"status", "success"}, {"user", user}, {"msg", "JWT verified"}};
            cb(resp.dump() + "\n");
            return Result::Success();
        } catch (const DbException& e) {
...
class GetTaskStatusStatement : public Statement {
   ...
   public:
    ...
    Result execute(HierarchyManager&, const std::string&,
        ...
        if (!g_async_manager)
            return Result::Error(StatusCode::INTERNAL_ERROR,
                                 "AsyncManager недоступен");

        auto r = g_async_manager->fetch_result(guid_);

        if (r.status == AsyncStatus::PENDING ||
            r.status == AsyncStatus::PROCESSING) {
...
class AlterUserAddToGroupStatement : public Statement {
   ...
   public:
    ...
    Result execute(HierarchyManager& hm, const std::string& token,
        ...
        auto res =
            hm.resolveTablePath("_system.user_groups", g_current_node_id);
        if (!res.isOk())
            return Result::Error(StatusCode::INTERNAL_ERROR,
                                 "RBAC не инициализирован");
...
class GrantPermStatement : public Statement {
   ...
   public:
    ...
    Result execute(HierarchyManager& hm, const std::string& token,
        ...
        auto res =
            hm.resolveTablePath("_system.permissions", g_current_node_id);
        if (!res.isOk())
            return Result::Error(StatusCode::INTERNAL_ERROR,
                                 "RBAC не инициализирован");
    ...
    Result execute(HierarchyManager& hm, const std::string& token,
        ...
        for (const auto& act : actions_) {
            auto cond = std::make_unique<LogicalNode>(
                "AND",
                std::make_unique<ComparisonNode>("grantee",
                                                 "==", Value(grantee_)),
                std::make_unique<LogicalNode>(
                    "AND",
                    std::make_unique<ComparisonNode>(
                        "is_group", "==", Value(is_group_ ? 1 : 0)),
                    std::make_unique<LogicalNode>(
                        "AND",
                        std::make_unique<ComparisonNode>("db_name",
                                                         "==", Value(db_)),
...
class AlterUserAddPermStatement : public GrantPermStatement {
   public:
    AlterUserAddPermStatement(std::string u, std::string db,
...
class AlterGroupAddPermStatement : public GrantPermStatement {
   public:
    AlterGroupAddPermStatement(std::string g, std::string db,
                               std::vector<std::string> p)
...
class AlterGroupDelPermStatement : public RevokePermStatement {
   public:
    AlterGroupDelPermStatement(std::string g, std::string db,
                               std::vector<std::string> p)
