...
class SQLParser {
   public:
    ...
    Result process(const std::string& query, HierarchyManager& hm,
                   const std::string& token, OutputCallback callback);
    ...
    int findColumnIndex(const TableHeader& header,
                        const std::string& colName) const;

    bool applyConstraints(Row& row, const TableHeader& header,
   ...
   private:
    ...
    void validateIdentifier(const std::string& name,
