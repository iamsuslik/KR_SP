...
Value SQLParser::parseLiteral(const std::string& token) {
  if (token.empty() || toUpper(token) == "NULL") return Value();
  ...
  int int_val = 0;
  auto [ptr, ec] =
      std::from_chars(token.data(), token.data() + token.size(), int_val);
...
TokenType SQLParser::getTokenType(const std::string& value) const {
  ...
  static const std::unordered_set<std::string> kKeywords = {
      ...
      "VALUES",     "CREATE",      "TABLE",  "DATABASE", "DROP",     "USE",
      "UPDATE",     "SET",         "DELETE", "AND",      "OR",       "NOT",
      "BETWEEN",    "LIKE",        "INT",    "STR",      "NOT_NULL", "INDEXED",
      "DEFAULT",    "AS",          "NULL",   "SUM",      "COUNT",    "AVG",
      "AUTH",       "USER",        "ALTER",  "GRANT",    "REVOKE",   "GROUP",
      "PERMISSION", "PERMISSIONS", "GET",    "REVERT",   "TO",       "ADD",
      "REMOVE"};
...
std::vector<Token> SQLParser::lex(const std::string& query) {
  ...
  for (std::size_t i = 0; i < query.size(); ++i) {
    ...
    if (c == ';') {
      pushToken();
      result.push_back({TokenType::SEMICOLON, ";"});
      continue;
    }
    if (c == '.') {
      pushToken();
    ...
    if (c == '*') {
      pushToken();
      result.push_back({TokenType::STAR, "*"});
      continue;
    }
...
Result SQLParser::process(const std::string& query, HierarchyManager& hm,
                          const std::string& token, OutputCallback cb) {
  try {
    if (g_async_manager) {
      const std::string active_db = g_async_manager->get_session_db(token);
      if (!active_db.empty()) {
        auto _r2 = hm.useDatabase(active_db);
        (void)_r2;
      }
    }
    ...
    for (const auto& tok : token_stream) {
      ...
      if (hasUpper && hasLower)
        return Result::Error(
            StatusCode::SYNTAX_ERROR,
            "Запрещён смешанный регистр в ключевом слове: '" + tok.value + "'");
    }

    auto stmt = parseStatement();
    if (!stmt) return Result::Error(StatusCode::SYNTAX_ERROR, "Пустой запрос");
...
std::unique_ptr<Statement> SQLParser::parseSelect() {
  ...
  if (!projRes.isOk()) throw DbException(projRes.code, projRes.details);
  ...
  if (match(TokenType::KEYWORD, "WHERE")) whereClause = parseExpression();
...
Result SQLParser::parseProjection(std::vector<std::string>& selectedCols,
    ...
    if (up == "SUM" || up == "COUNT" || up == "AVG") {
      ...
      if (up == "COUNT" && match(TokenType::STAR)) {
        ...
        arg = expect(TokenType::IDENTIFIER,
                     "Ожидалось имя колонки внутри " + up + "(...)")
                  .value;
      ...
      AggregateType type = (up == "SUM")   ? AggregateType::SUM
  ...
  parseOneColumn();
  while (match(TokenType::COMMA)) parseOneColumn();
...
std::unique_ptr<Statement> SQLParser::parseInsert() {
  ...
  if (match(TokenType::KEYWORD, "VALUE") ||
  ...
  if (toUpper(peek().value) == "SELECT") {
    ...
    match(TokenType::SEMICOLON);
    return std::make_unique<InsertSelectStatement>(tableName, columns,
                                                   std::move(subquery));
  }
...
std::unique_ptr<Statement> SQLParser::parseUpdate() {
  ...
  parseAssignment();
  while (match(TokenType::COMMA)) parseAssignment();

  std::unique_ptr<ASTNode> whereClause;
  if (match(TokenType::KEYWORD, "WHERE")) whereClause = parseExpression();

  match(TokenType::SEMICOLON);
  return std::make_unique<UpdateStatement>(tableName, std::move(assignments),
...
std::unique_ptr<Statement> SQLParser::parseCreate() {
  ...
  if (target == "TABLE") {
    ...
    if (peek().type == TokenType::DOT) {
      consume();
      const std::string part2 =
          expect(TokenType::IDENTIFIER, "Ожидалось имя таблицы после '.'")
              .value;
    ...
    if (!colRes.isOk()) throw DbException(colRes.code, colRes.details);
...
Result SQLParser::parseColumnDefinitions(std::vector<ColumnDef>& outCols) {
  ...
  do {
    ...
    const std::string colTypeStr = toUpper(
        expect(TokenType::KEYWORD, "Ожидался тип колонки INT или STR").value);
    if (colTypeStr != "INT" && colTypeStr != "STR")
      throw DbException(
          StatusCode::SYNTAX_ERROR,
          "Неизвестный тип колонки '" + colTypeStr + "'. Допустимы INT и STR.");

    ColumnDef cd(colName, colTypeStr == "INT" ? DataType::INT : DataType::STR);
...
std::unique_ptr<Statement> SQLParser::parseAuth() {
  ...
  if (match(TokenType::KEYWORD, "JWT")) {
    std::string token =
        expect(TokenType::STRING_LITERAL, "Ожидался JWT токен").value;
...
std::unique_ptr<Statement> SQLParser::parseGet() {
  ...
  match(TokenType::SEMICOLON);
  if (guid.empty())
    throw DbException(StatusCode::SYNTAX_ERROR, "GET: ожидался GUID задачи");
...
std::unique_ptr<Statement> SQLParser::parseAlter() {
  ...
  if (target == "USER") {
    ...
    if (match(TokenType::KEYWORD, "ADD")) {
      if (match(TokenType::KEYWORD, "TO")) {
        ...
        match(TokenType::SEMICOLON);
        return std::make_unique<AlterUserAddToGroupStatement>(username, dbName,
                                                              groupName);
      }
      ...
      if (toUpper(peek().value) == "PERMISSION" ||
        ...
        match(TokenType::SEMICOLON);
        return std::make_unique<AlterUserAddPermStatement>(username, dbName,
                                                           std::move(perms));
      }
    }
    throw DbException(StatusCode::SYNTAX_ERROR,
  ...
  if (target == "GROUP") {
    ...
    if (match(TokenType::KEYWORD, "FROM")) {
      ...
      if (isAdd)
        return std::make_unique<AlterGroupAddPermStatement>(groupName, dbName,
                                                            std::move(perms));
      else
        return std::make_unique<AlterGroupDelPermStatement>(groupName, dbName,
                                                            std::move(perms));
    }
    throw DbException(StatusCode::SYNTAX_ERROR,
...
std::unique_ptr<ASTNode> SQLParser::parseExpression() {
  ...
  while (match(TokenType::KEYWORD, "OR")) {
    ...
    node =
        std::make_unique<LogicalNode>("OR", std::move(node), std::move(right));
  }
  return node;
}
...
std::unique_ptr<ASTNode> SQLParser::parseAndExpr() {
  ...
  while (match(TokenType::KEYWORD, "AND")) {
    ...
    node =
        std::make_unique<LogicalNode>("AND", std::move(node), std::move(right));
  }
  return node;
}
...
std::unique_ptr<ASTNode> SQLParser::parsePrimary() {
  ...
  if (match(TokenType::KEYWORD, "BETWEEN")) {
    ...
    expect(TokenType::KEYWORD,
           "Ожидалось AND в конструкции BETWEEN ... AND ...");

    const Token highTok = consume();
    Value high = parseLiteral(highTok.value);

    return std::make_unique<BetweenNode>(lhsName, std::move(low),
                                         std::move(high));
  }
  ...
  if (match(TokenType::KEYWORD, "LIKE")) {
    ...
    return std::make_unique<ComparisonNode>(lhsName, "LIKE",
                                            std::move(pattern));
  }
...
Value SQLParser::parseValue() { return parseLiteral(consume().value); }

int SQLParser::findColumnIndex(const TableHeader& header,
...
bool SQLParser::prepareAndValidateRow(
    Row& outRow, const TableHeader& header,
    const std::vector<std::string>& targetCols,
    const std::vector<std::string>& rawValues, OutputCallback cb) {
  if (targetCols.empty()) {
    if (rawValues.size() > header.column_count) {
      cb("[Ошибка] Передано больше значений (" +
         std::to_string(rawValues.size()) + "), чем колонок в таблице (" +
    ...
    if (targetCols.size() != rawValues.size()) {
      cb("[Ошибка] Количество колонок (" + std::to_string(targetCols.size()) +
