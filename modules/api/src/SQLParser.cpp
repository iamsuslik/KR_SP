    ...
    if (!isValidIdentifier(name))
        throw DbException(StatusCode::SYNTAX_ERROR,
                          "Недопустимое имя '" + name +
                              "' в контексте: " + context +
                              ". Допустимы латинские буквы, цифры и '_'; не "
                              "начинается с цифры.");
}
...
TokenType SQLParser::getTokenType(const std::string& value) const {
    ...
    static const std::unordered_set<std::string> kKeywords = {
        ...
        "VALUE",  "VALUES",     "CREATE",      "TABLE",   "DATABASE",
        "DROP",   "USE",        "UPDATE",      "SET",     "DELETE",
        "AND",    "OR",         "NOT",         "BETWEEN", "LIKE",
        "INT",    "STR",        "NOT_NULL",    "INDEXED", "DEFAULT",
        "AS",     "NULL",       "SUM",         "COUNT",   "AVG",
        "AUTH",   "USER",       "ALTER",       "GRANT",   "REVOKE",
        "GROUP",  "PERMISSION", "PERMISSIONS", "GET",     "REVERT",
        "TO",     "ADD",        "REMOVE"};
...
Result SQLParser::process(const std::string& query, HierarchyManager& hm,
    ...
    try {
        if (g_async_manager) {
            const std::string active_db =
                g_async_manager->get_session_db(token);
            if (!active_db.empty()) {
    ...
    auto parseOneColumn = [&]() {
        ...
        if (up == "SUM" || up == "COUNT" || up == "AVG") {
            ...
            expect(TokenType::LPAREN,
                   "Ожидалось '(' после агрегатной функции " + up);
            ...
            if (match(TokenType::KEYWORD, "AS"))
                alias =
                    expect(TokenType::IDENTIFIER, "Ожидалось имя псевдонима")
                        .value;
...
std::unique_ptr<Statement> SQLParser::parseUpdate() {
    ...
    auto parseAssignment = [&]() {
        ...
        expect(TokenType::OPERATOR,
               "Ожидался оператор '=' после имени колонки");
...
std::unique_ptr<Statement> SQLParser::parseCreate() {
    ...
    if (target == "TABLE") {
        ...
        match(TokenType::SEMICOLON);
        return std::make_unique<CreateTableStatement>(tableName,
                                                      std::move(cols));
    }
...
Result SQLParser::parseColumnDefinitions(std::vector<ColumnDef>& outCols) {
    ...
    do {
        ...
        const std::string colTypeStr = toUpper(
            expect(TokenType::KEYWORD, "Ожидался тип колонки INT или STR")
                .value);
    ...
    if (target == "DATABASE") {
        ...
        if (match(TokenType::KEYWORD, "ADD")) {
            if (match(TokenType::KEYWORD, "GROUP")) {
                ...
                match(TokenType::SEMICOLON);
                return std::make_unique<AlterDbAddGroupStatement>(dbName,
                                                                  gName);
            }
        }
        throw DbException(StatusCode::SYNTAX_ERROR,
...
std::unique_ptr<ASTNode> SQLParser::parseExpression() {
    ...
    while (match(TokenType::KEYWORD, "OR")) {
        ...
        node = std::make_unique<LogicalNode>("OR", std::move(node),
                                             std::move(right));
    }
    return node;
}
...
std::unique_ptr<ASTNode> SQLParser::parseAndExpr() {
    ...
    while (match(TokenType::KEYWORD, "AND")) {
        ...
        node = std::make_unique<LogicalNode>("AND", std::move(node),
                                             std::move(right));
    }
    return node;
}
...
std::unique_ptr<ASTNode> SQLParser::parsePrimary() {
    ...
    return std::make_unique<ComparisonNode>(lhsName, opTok.value,
                                            std::move(rhs));
}
